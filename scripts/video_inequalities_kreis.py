...
def run():
	...
	b1 = lambda x: np.array( (scene.measurements['joint_1'] + x[0]) - 2.8407 )
	b2 = lambda x: np.array( 4.41149633 - (scene.measurements['joint_1'] + x[0]) )

	b3 = lambda x: np.array( (scene.measurements['joint_2'] + x[1]) + 1.9 )
	...
	#self.inequaliy_list = [b16]
	solver.add_inequality([b3])
	#solver.add_inequality([b3,b4])
